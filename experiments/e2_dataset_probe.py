...
def sweep_episode(predictor, fob, base_sample, Np_list, H, W, dummy_model, qry_img_canonical):
    ...
    try:
        ...
        # Monkey patch its allocate method to save 'a'
...
def main():
    print("Running E2 Dataset Probe...")  # CHANGED
    device = "cuda"
    ...
    out_csv = "results/e2_dataset_probe.csv"  # CHANGED
    
    Np_list = [0, 1, 2, 4, 6, 8, 10, 12, 16, 20, 24]
    
    # 1. Load data
    volumes, stats = load_volumes(data_root, (-1024, 3072), None)
    organ_map = {4: "gallbladder", 7: "stomach", 8: "aorta", 9: "ivc"}  # CHANGED
    organs = available_organs(volumes, organ_map, 100)  # CHANGED: min_pixels=100
    ...
    results = []
    skipped = 0
    episodes = 100  # CHANGED: probe only
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        for ep_i in tqdm(range(episodes), desc="Sweeping Np"):
            ...
            ep = sample_episode(volumes, cls, 100)  # CHANGED: min_pixels=100
            if ep is None:
                skipped += 1
                continue
    ...
    print(f"Probe complete. Wrote {episodes - skipped} valid episodes to {out_csv}")
