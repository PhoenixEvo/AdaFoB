...
foreground prototype and the support background prototypes. The prompt budget
is then dynamically computed via:
\begin{equation}
  N_{p} \;=\; \mathrm{clip}\!\left(\left\lfloor \nu \cdot e^{-a / a_0} \cdot L \cdot (1 + \lambda\,\overline{|\kappa|}) \right\rceil,\; 0,\; 24\right),
  \label{eq:budget}
\end{equation}
where $\nu$ is the base density, $a_0$ scales the ambiguity penalty, and
$\lambda$ weights the curvature term. Because $N_{p}$ emerges from measurable
geometry and feature-space ambiguity rather than a fixed scalar, the budget is
interpretable and dynamically scales down (even to $N_{p}{=}0$) when the model
is highly confident ($a \to 0$) or the target is extremely small.

Because Eq.~\eqref{eq:budget} explicitly accommodates $\lambda$, we show through
Grid-Search that different semantic classes inherently prefer different
geometric multipliers. For thin, highly sensitive structures like the aorta,
the model suppresses curvature signals ($\lambda=0.0$). For larger,
complex shapes like the gallbladder, the model maximizes curvature attention
($\lambda=1.0$).
